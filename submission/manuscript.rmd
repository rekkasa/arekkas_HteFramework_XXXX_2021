...
characteristics [@Varadhan2013]. One promising approach is “risk modeling”, in
which treatment effects are estimated in strata of predicted risk [@Kent2010;
@Kent2018]. Such a risk-based approach first stratifies patients according to
baseline risk predictions, using either an existing or an internally developed
risk prediction model [@Kent2010]. Then, relative and absolute treatment
effects are estimated within risk strata.
...
the treatment arms in order to avoid inducing spurious interactions
[@Burke2014; @vanKlaveren2019]. To do this, we develop the patient-level
prediction model in the propensity score-matched subset of the population (1:1),
where treatment assignment is well-balanced. The propensity scores are based on
LASSO logistic regression for modeling the association between treatment
assignment and all available demographics, drug exposures, diagnoses,
measurements and medical procedures. Finally, we need to define the time horizon
within which we aim to make predictions and we also need to select the
machine-learning algorithm we want to use to generate patient-level predictions.
Currently, the available options are regularized logistic regression, random
forest, gradient boosting machines, decision tree, naive Bayes, K-nearest
neighbors, neural network and deep learning (convolutional neural networks,
recurrent neural network and deep nets).
...
patients should also be reported. This is quite relevant as the performance of
...
Lower event rate and higher c-statistic (given good calibration) result in high
risk heterogeneity, thus making estimated average treatment effects
uninformative. In this case, risk stratified analysis of HTE can be more
...
data sources. This also enables the development of scalable demonstration tools
such as R-shiny applications, interactive tables and figures.
...
for clinical interpretation, it is not appropriate to guide clinical practice,
where decisions need to be made at the individual level [@Kent2019].
Presentation of treatment effects as a continuous function of risk would be more
helpful, but is methodologically challenging. Future research is necessary for
the development of methods for continuous risk-based assessment of HTE.
...
possibly better fit to the larger treatment arm, this may introduce spurious
